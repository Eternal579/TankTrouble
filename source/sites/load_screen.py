...
class LoadScreen:
    ...
    def update(self, surface, keys):
        ...
        surface.blit(tools.create_textImg('PLAYERS'),
                     (C.SCREEN_W*0.43, C.SCREEN_H*0.42))
        if self.score[0] != -1:
            surface.blit(tools.create_image(setup.GRAPHICS['red'], 0, 0, C.PLAYER_PY, C.PLAYER_PX,
                                         C.CURSOR_MULTI), (C.SCREEN_W*0.27, C.SCREEN_H*0.48))
        if self.score[1] != -1:
            surface.blit(tools.create_image(setup.GRAPHICS['green'], 0, 0, C.PLAYER_PY, C.PLAYER_PX,
                                         C.CURSOR_MULTI), (C.SCREEN_W*0.47, C.SCREEN_H*0.48))
        if self.score[2] != -1:
